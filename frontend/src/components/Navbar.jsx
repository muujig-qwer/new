...
export default function Navbar() {
  // ...existing code...
  const [showMobileCart, setShowMobileCart] = useState(false); // mobile cart modal toggle
  const mobileCartRef = useRef(null);
  // Close mobile cart modal when clicking outside (mobile only)
  useEffect(() => {
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex-1 max-w-2xl mx-8 md:mx-4">
              <form onSubmit={handleSearch} className="flex w-full">
                ...
                {/* Mobile: жижиг search bar зүүн талд үргэлж харагдана */}
                <div className="sm:hidden flex items-center w-full">
                  <input
                    ...
                    onChange={(e) => setSearchQuery(e.target.value)}
                    placeholder="Хайлт..."
                    className="flex-1 min-w-0 px-3 py-1 border border-gray-300 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
                    onFocus={() => setShowMobileCart(true)}
                  />
                </div>
              </form>
            </div>
            ...
            <div className={`flex items-center gap-6 sm:flex ${showMobileCart ? "hidden sm:flex" : ""}`}>
      {/* Mobile Search Overlay */}
      {showMobileSearch && (
        <div
          className="fixed inset-0 z-[100] bg-black bg-opacity-40 flex items-center justify-center sm:hidden"
          onClick={() => setShowMobileSearch(false)}
        >
          <div
            className="bg-white rounded-xl shadow-lg w-[95vw] max-w-md mx-auto p-4 flex flex-col"
          ...
          >
            ...
            <button
              className="w-full bg-green-600 text-white rounded-full py-2 font-semibold hover:bg-green-700 transition-colors"
              onClick={(e) => {
                e.preventDefault();
                  ...
                  <>
                    {!isAdmin && (
                      <>
                        ...
                        <Link
                          href="/cart"
                          className={`relative p-2 rounded-2xl transition-colors min-w-[40px] justify-center items-center bg-white hidden sm:flex ${cartItems.length === 0 ? "text-gray-600" : "text-black"}`}
                        >
                          {cartItems.length === 0 ? (
                            <FaShoppingCart className="h-5 w-5" />
                          ) : (
        ...
        >
          ...
          >
            <div className="flex items-center mb-2 justify-between">
              <div className="font-bold text-lg">Таны сагс</div>
              <button
            ...
            {cartItems.length === 0 ? (
              <div className="text-center text-gray-400 py-8">Сагс хоосон байна.</div>
            ) : (
              <div className="flex-1 overflow-y-auto max-h-60 divide-y">
                {cartItems.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-3 py-2">
                    <div className="w-14 h-14 bg-gray-100 rounded-lg flex items-center justify-center overflow-hidden">
                      {item.images && item.images[0] ? (
                        <img src={item.images[0].startsWith('http') ? item.images[0] : `/uploads/${item.images[0]}`} alt={item.name} className="w-full h-full object-cover" />
                      ) : (
            ...
            <div className="mt-4 flex flex-col gap-2">
              ...
              <Link
                href="/cart"
                className="w-full bg-green-600 text-white rounded-full py-2 font-semibold hover:bg-green-700 text-center mt-2"
                onClick={() => setShowMobileCart(false)}
              >
                Сагс руу очих
              </Link>
